...
async def test_update_service(client, test_db):
    ...
    result = await test_db.execute(select(User).where(User.email == "provider1@test.com"))
    provider = result.scalars().first()
    provider.role = "provider"
    await test_db.commit()

    login_resp = await client.post("/api/v1/auth/login", data={
        "username": "provider1@test.com", "password": "secret123",
    })
    ...
    # Actualizar servicio
    update_resp = await client.put(f"/api/v1/services/{service_id}", json={
...
async def test_delete_service(client, test_db):
    ...
    result = await test_db.execute(select(User).where(User.email == "provider2@test.com"))
    provider = result.scalars().first()
    provider.role = "provider"
    await test_db.commit()

    login_resp = await client.post("/api/v1/auth/login", data={
        "username": "provider2@test.com", "password": "secret123",
    })
    token = login_resp.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
...
async def test_unauthorized_service_update(client, test_db):
    ...
    login_resp = await client.post("/api/v1/auth/login", data={
        "username": "owner@test.com", "password": "secret123",
    })
    token = login_resp.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
    ...
    client_login = await client.post("/api/v1/auth/login", data={
        "username": "intruso@test.com", "password": "secret123",
    })
