...
@pytest.mark.anyio
async def test_send_reminder_email_calls_send_email():
    with patch("app.tasks.email_tasks.send_email") as mock_send:
        send_reminder_email(2, "recordatorio@test.com")
        mock_send.assert_called_once_with(
            "recordatorio@test.com",
            "Recordatorio de tu reserva #2",
            "<p>Te recordamos que tienes una reserva activa con ID 2.</p>",
        )